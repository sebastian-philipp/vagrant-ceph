...
if INSTALLATION == 'salt'
    hosts = Vagrant::Hosts.new(yml_config[CONFIGURATION]['nodes'], selected = 'public',
                               domain='ceph', aliases={'admin' => 'salt'})
...
Vagrant.configure(2) do |config|
    ...
    if ENV['DISTRO'] == 'debian'
        config.vm.box = 'debian/jessie64' # should't work atm.
    else
    ...
    nodes = yml_config[CONFIGURATION]['nodes'].keys.reject { |i| i == 'admin' }
